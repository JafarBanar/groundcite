...
`groundcite` runs after the model has spoken: it extracts every citation from the answer,
...
`groundcite` makes the failure visible and lets the pipeline say no.

## Use

```python
from groundcite import validate, guard, NUMERIC
...
```bash
groundcite check answer.md --sources 1 2 3 4 5
# OK  citations=3 resolved=3 coverage=1.00 (4/4)

groundcite check answer.md --sources 1 2 3 --pattern bracket-id --json
...
model or a human. `groundcite` guarantees the cheaper, sharper property: nothing is
